  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    */
    if (prevState !== this.state && prevState.masv !== "") {
      ...
      let findIndex = this.props.mainData.findIndex(
        (item) => item.masv === this.state.masv
      );
  ...
  render() {
    ...
    return (
      <div className="container my-5">
        ...
        <table className="table table-striped table-white my-2">
          <thead className="bg-dark text-white">
            <tr>
              <th className="text-center">Mã số sv</th>
              {dataForm.title.map((item, index) => {
                return (
                  <th
                    className="text-center"
                    key={index}
                    width={index === 0 ? "25%" : undefined}
                    colSpan={index === 2 ? 2 : undefined}
                  >
                    {item}
                  </th>
                );
